...
export function RetroCampaign({save,selectedMap,setSelectedMap,onStart,onBack,onCodex}){
 const retro=save.worlds.retro;const map=retroMaps[selectedMap-1];const complete=retro.completedMap??0;const nextUnlock=useMemo(()=>nextRetroTowerUnlock(retro),[retro.completedMap,retro.cartridges]);const tower=nextUnlock?retroTowers.find(t=>t.id===nextUnlock.id):null;
 return <section className="retro-campaign">
  <header className="retro-header">
   ...
   <div className="retro-counters"><span>📼 {retro.cartridges??0}/75</span><span>🏆 {(retro.highScore??0).toLocaleString()}</span><span>🔥 x{retro.bestCombo??1}</span><button onClick={onCodex}>Field Guide</button><button onClick={onBack}>← Stone Age</button></div>
  </header>
