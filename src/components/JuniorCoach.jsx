...
const GUIDES={'stone-age':{name:'Koda',icon:'🐾',title:'Trail Scout'},retro:{name:'Byte',icon:'👾',title:'Arcade Buddy'},future:{name:'Nova',icon:'🤖',title:'Grid Guide'},space:{name:'Comet',icon:'🚀',title:'Flight Buddy'},'time-rift':{name:'Echo',icon:'⏳',title:'Time Guide'}};
const junior=s=>s?.settings?.juniorMode!==false;
function era(){const h=(location.hash||'').replace('#','');return GUIDES[h]?h:'stone-age'}
function state(){
...
export function JuniorCoach(){
 const[settings,setSettings]=useState(()=>loadSave().settings??{}),[tip,setTip]=useState(state),[hidden,setHidden]=useState(false),[world,setWorld]=useState(era);
 const enabled=junior({settings});
 useEffect(()=>{const f=e=>{const s=e.detail?.save??loadSave();setSettings(s.settings??{});if(junior(s))setHidden(false)};const hash=()=>setWorld(era());window.addEventListener('chrono:save',f);window.addEventListener('hashchange',hash);return()=>{window.removeEventListener('chrono:save',f);window.removeEventListener('hashchange',hash)}},[]);
 ...
 const guide=GUIDES[world]??GUIDES['stone-age'];
 const label=useMemo(()=>tip.kind==='campaign'?`${guide.name} • MAP HELPER`:`${guide.name} • ${guide.title}`,[tip.kind,guide.name,guide.title]);
 const canSpeak=typeof window!=='undefined'&&'speechSynthesis'in window&&settings.readAloud!==false;
 const speak=()=>{if(!canSpeak)return;window.speechSynthesis.cancel();const utterance=new SpeechSynthesisUtterance(`${guide.name} says: ${tip.title}. ${tip.body}`);utterance.rate=.92;utterance.pitch=1.05;window.speechSynthesis.speak(utterance)};
 if(!enabled)return null;
 if(hidden)return <button className="junior-help-bubble" onClick={()=>setHidden(false)} aria-label={`Show ${guide.name} game coach`}>{guide.icon} <span>Help</span></button>;
 return <aside className={`junior-coach junior-${tip.kind}`} aria-live="polite"><button className="junior-close" onClick={()=>setHidden(true)} aria-label="Hide game coach">×</button><span className="junior-face">{guide.icon}</span><div><small>{label}</small><b>{tip.title}</b><p>{tip.body}</p>{canSpeak&&<button className="junior-speak" onClick={speak}>🔊 {guide.name} reads this</button>}</div></aside>;
}
