...
function floatDamage(rect,amount,boss=false){const node=document.createElement('div');node.className=`chrono-damage-float ${boss?'boss':''}`;node.textContent=`−${Math.max(1,Math.round(amount))}%`;node.style.left=`${rect.left+rect.width/2}px`;node.style.top=`${rect.top+Math.max(4,rect.height*.18)}px`;document.body.appendChild(node);removeLater(node,620)}
function tracer(from,to,boss=false){if(document.documentElement.classList.contains('effects-low')||document.documentElement.classList.contains('chrono-reduced-motion'))return;const dx=to.x-from.x,dy=to.y-from.y,len=Math.hypot(dx,dy),angle=Math.atan2(dy,dx)*180/Math.PI;const line=document.createElement('div');line.className=`chrono-tracer ${boss?'boss':''}`;line.style.left=`${from.x}px`;line.style.top=`${from.y}px`;line.style.width=`${len}px`;line.style.transform=`rotate(${angle}deg)`;document.body.appendChild(line);removeLater(line,170)}
function impact(enemy,boss=false){enemy.classList.remove('chrono-hit');void enemy.offsetWidth;enemy.classList.add('chrono-hit');setTimeout(()=>enemy.classList.remove('chrono-hit'),160);if(boss)document.body.classList.add('chrono-boss-impact');setTimeout(()=>document.body.classList.remove('chrono-boss-impact'),120)}
function defeatBurst(rect,boss=false){if(document.documentElement.classList.contains('effects-low'))return;const node=document.createElement('div');node.className=`chrono-defeat-burst ${boss?'boss':''}`;node.style.left=`${rect.left+rect.width/2}px`;node.style.top=`${rect.top+rect.height/2}px`;node.innerHTML='<i></i><i></i><i></i><i></i><i></i><i></i>';document.body.appendChild(node);removeLater(node,boss?700:430)}
...
export function CombatJuiceBridge(){
 useEffect(()=>{
  ...
  return()=>{observer.disconnect();document.querySelectorAll('.chrono-tracer,.chrono-damage-float,.chrono-defeat-burst').forEach(n=>n.remove());document.body.classList.remove('chrono-boss-impact')};
 },[]);
 return null;
}
