...
export function SaveManager({save,onReplace,onClose}){
 const inputRef=useRef(null);const[message,setMessage]=useState('Back up your progress before moving devices or clearing browser storage.');const[dangerArmed,setDangerArmed]=useState(false);const[restoreArmed,setRestoreArmed]=useState(false);const[meta]=useState(()=>loadSaveMeta());const backupAvailable=hasBackup();
 ...
 const reset=()=>{if(!dangerArmed){setDangerArmed(true);setMessage('Press Reset Progress again to confirm. Export a backup first if you may want this progress later.');return}const fresh=clearSaveStorage();onReplace(fresh??defaultSave());setDangerArmed(false);setRestoreArmed(false);setMessage('All local Chrono Defense progress and the rolling backup were reset.')};
 ...
 return <div className="save-overlay" onClick={onClose}><section className="save-panel" onClick={event=>event.stopPropagation()}>
  <header><div><small>LOCAL PLAYER DATA</small><h2>Save & Backup</h2></div><button onClick={onClose} aria-label="Close save manager">×</button></header>
  ...
  <div className="save-actions">
   ...
   <button className={dangerArmed?'danger armed':'danger'} onClick={reset}><span>⚠️</span><div><b>{dangerArmed?'Confirm Full Reset':'Reset Progress'}</b><small>{dangerArmed?'Press again to delete current progress and its rolling backup.':'Start Stone Age over from the beginning.'}</small></div></button>
  </div>
