...
export function RetroExperience({onSwitchWorld}){
 ...
 useEffect(()=>persistSave(save),[save]);useEffect(()=>setEraChallenge('retro',challenge),[challenge]);
 useEffect(()=>{if(!retro.unlocked)onSwitchWorld?.('stone-age')},[retro.unlocked,onSwitchWorld]);
 const start=()=>{setEraChallenge('retro',challenge);setBattleKey(v=>v+1);setScreen('battle')};const back=()=>onSwitchWorld?.('stone-age');const future=()=>onSwitchWorld?.('future');const exit=()=>{setSelectedMap(Math.min(25,Math.max(1,save.worlds.retro.highestMap??1)));setScreen('campaign')};const next=map=>{setSelectedMap(map);setEraChallenge('retro',challenge);setBattleKey(v=>v+1);setScreen('battle')};const finishTutorial=()=>setSave(prev=>({...prev,worlds:{...prev.worlds,retro:{...prev.worlds.retro,tutorialComplete:true}}}));
 return <>{screen==='campaign'?<RetroCampaign save={save} selectedMap={selectedMap} setSelectedMap={setSelectedMap} challenge={challenge} setChallenge={setChallenge} onStart={start} onBack={back} onFuture={future} onCodex={()=>setShowCodex(true)}/>:<RetroBattle key={`${selectedMap}-${challenge}-${battleKey}`} mapNumber={selectedMap} save={save} setSave={setSave} onExit={exit} onNextMap={next}/>} {!retro.tutorialComplete&&<RetroTutorial onComplete={finishTutorial}/>} {showCodex&&<RetroCodex retroSave={retro} onClose={()=>setShowCodex(false)}/>}</>;
}
