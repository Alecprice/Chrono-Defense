...
function clearPauseClasses(activeRoot=null){CONFIGS.forEach(c=>document.querySelectorAll(c.root).forEach(root=>{if(root!==activeRoot)root.classList.remove('chrono-battle-paused')}))}
export function WaveStatusBridge(){useEffect(()=>{let last='';const timer=setInterval(()=>{const found=active();if(!found){clearPauseClasses();return}const{config,root}=found,host=root.querySelector(config.host);if(!host)return;const threats=root.querySelectorAll(config.enemy).length,start=root.querySelector(config.start),result=resultState(root),paused=isPaused(root,threats);clearPauseClasses(root);root.classList.toggle('chrono-battle-paused',paused);let state='READY',tone='ready';if(result){state=result;tone=result==='VICTORY'?'victory':result==='DEFEAT'?'defeat':'ready'}else if(paused){state='PAUSED';tone='paused'}else if(start?.disabled&&threats){state='ENGAGED';tone='engaged'}else if(start?.disabled){state='DEPLOYING';tone='engaged'}const key=`${config.root}:${state}:${threats}`;if(key===last)return;last=key;const ribbon=ensure(host);ribbon.className=`chrono-wave-ribbon ${tone}`;ribbon.innerHTML=`<i></i><b>${state}</b><span>${threats?`${threats} ${threats===1?'THREAT':'THREATS'}`:'BUILD PHASE'}</span>`},180);return()=>{clearInterval(timer);document.querySelectorAll('.chrono-wave-ribbon').forEach(n=>n.remove());clearPauseClasses()}},[]);return null}
