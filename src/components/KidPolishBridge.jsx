...
import{armAdaptiveAssist,clearAdaptiveAssist}from'../core/adaptiveAssist.js';
...
const LOSS_RE=/GAME OVER|CORE OFFLINE|COLONY LOST|TIMELINE COLLAPSE|Village Lost|The village has fallen|Defeat/i;
...
export function KidPolishBridge(){const[on,setOn]=useState(()=>junior()),[battle,setBattle]=useState(()=>detectBattle()),[bossPrep,setBossPrep]=useState(null),[help,setHelp]=useState(false),[adaptiveCard,setAdaptiveCard]=useState(null),[assistArmed,setAssistArmed]=useState(false),[welcome,setWelcome]=useState(()=>firstRun());
 ...
 return <>
  ...
  {adaptiveCard&&<div className="adaptive-help-card"><small>💛 COACH HELP</small><h3>This map is tricky — that’s okay.</h3><p>You’ve tried this map {adaptiveCard.losses} times. You can keep normal difficulty, or make only your next attempts on this map a little gentler.</p><div><span>⭐ Put your strongest tower where it sees the road longest.</span><span>⬆️ Upgrade 2 good towers before adding lots of weak ones.</span><span>🪙 Keep a little money/resources between waves.</span></div><button onClick={()=>{armAdaptiveAssist(adaptiveCard.world,adaptiveCard.map.number);setAssistArmed(true);setAdaptiveCard(null)}}>💛 Make This Map Easier</button><button onClick={()=>{setHelp(true);setAdaptiveCard(null)}}>Show Me What Works</button><button className="quiet" onClick={()=>setAdaptiveCard(null)}>Keep Normal Difficulty</button></div>}
  {assistArmed&&battle&&<div className="adaptive-assist-pill">💛 Gentle Retry Ready • {battle.map.name}</div>}
 </>;
}
