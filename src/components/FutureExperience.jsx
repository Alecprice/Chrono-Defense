...
export function FutureExperience({onSwitchWorld}){
 ...
 return <>{screen==='campaign'?<FutureCampaign save={save} selectedMap={selectedMap} setSelectedMap={setSelectedMap} challenge={challenge} setChallenge={setChallenge} onDaily={launchDaily} onStart={start} onBack={back} onSpace={space} onCodex={()=>setShowCodex(true)}/>:<FutureBattle key={`${selectedMap}-${challenge}-${battleKey}`} mapNumber={selectedMap} save={save} setSave={setSave} onExit={exit} onNextMap={next}/>} {!world.tutorialComplete&&<FutureTutorial onComplete={finishTutorial}/>} {showCodex&&<AdvancedEraCodex worldId="future" title="Future Field Guide" subtitle="GRID INTELLIGENCE DATABASE" towers={futureTowers} enemies={futureEnemies} bosses={futureBosses} onClose={()=>setShowCodex(false)}/>}</>;
}
