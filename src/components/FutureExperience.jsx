...
export function FutureExperience({onSwitchWorld}){
 ...
 const start=()=>{setEraChallenge('future',challenge);setBattleKey(v=>v+1);setScreen('battle')},back=()=>onSwitchWorld?.('retro'),space=()=>onSwitchWorld?.('space'),exit=()=>{setSelectedMap(Math.min(25,Math.max(1,save.worlds.future.highestMap??1)));setScreen('campaign')},next=map=>{setSelectedMap(map);setEraChallenge('future',challenge);setBattleKey(v=>v+1);setScreen('battle')},finishTutorial=()=>setSave(prev=>({...prev,worlds:{...prev.worlds,future:{...prev.worlds.future,tutorialComplete:true}}}));
 return <>{screen==='campaign'?<FutureCampaign save={save} selectedMap={selectedMap} setSelectedMap={setSelectedMap} challenge={challenge} setChallenge={setChallenge} onStart={start} onBack={back} onSpace={space} onCodex={()=>setShowCodex(true)}/>:<FutureBattle key={`${selectedMap}-${challenge}-${battleKey}`} mapNumber={selectedMap} save={save} setSave={setSave} onExit={exit} onNextMap={next}/>} {!world.tutorialComplete&&<FutureTutorial onComplete={finishTutorial}/>} {showCodex&&<AdvancedEraCodex worldId="future" title="Future Field Guide" subtitle="GRID INTELLIGENCE DATABASE" towers={futureTowers} enemies={futureEnemies} bosses={futureBosses} onClose={()=>setShowCodex(false)}/>}</>;
}
