...
export function FutureExperience({onSwitchWorld}){
 const[save,setSave]=useState(()=>loadSave()),world=save.worlds.future,[resumeCheckpoint,setResumeCheckpoint]=useState(()=>loadBattleCheckpoint('future')),[screen,setScreen]=useState(()=>world.tutorialComplete&&resumeCheckpoint?'resume':'campaign'),[selectedMap,setSelectedMap]=useState(()=>resumeCheckpoint?.mapNumber??Math.min(25,Math.max(1,world.highestMap??1))),[battleKey,setBattleKey]=useState(0),[showCodex,setShowCodex]=useState(false),[challenge,setChallenge]=useState(()=>resumeCheckpoint?.modeId??'normal'),battleSetSave=trackedEraBattleSetter(setSave,{worldId:'future',challenge,mapNumber:selectedMap});useEraActionStats(setSave,'future',screen==='battle');
 ...
 const start=()=>{clearBattleCheckpoint();setResumeCheckpoint(null);clearActiveDaily();setEraChallenge('future',challenge);setBattleKey(v=>v+1);setScreen('battle')},launchDaily=mission=>{clearBattleCheckpoint();setResumeCheckpoint(null);launchDailyMission(mission);setSelectedMap(mission.mapNumber);setChallenge(mission.challengeId);setBattleKey(v=>v+1);setScreen('battle')},back=()=>onSwitchWorld?.('retro'),space=()=>onSwitchWorld?.('space'),exit=()=>{clearBattleCheckpoint();setResumeCheckpoint(null);setSelectedMap(Math.min(25,Math.max(1,save.worlds.future.highestMap??1)));setScreen('campaign')},next=map=>{clearBattleCheckpoint();setResumeCheckpoint(null);clearActiveDaily();setSelectedMap(map);setEraChallenge('future',challenge);setBattleKey(v=>v+1);setScreen('battle')},resume=()=>setScreen('battle'),fresh=()=>{clearBattleCheckpoint();setResumeCheckpoint(null);setBattleKey(v=>v+1);setScreen('battle')},campaign=()=>{clearBattleCheckpoint();setResumeCheckpoint(null);setScreen('campaign')},finishTutorial=()=>setSave(prev=>({...prev,worlds:{...prev.worlds,future:{...prev.worlds.future,tutorialComplete:true}}}));
 if(!world.tutorialComplete)return <FutureTutorial onComplete={finishTutorial}/>;
 if(screen==='resume'&&resumeCheckpoint)return <ResumeBattlePrompt worldId="future" checkpoint={resumeCheckpoint} onContinue={resume} onFresh={fresh} onCampaign={campaign}/>;
