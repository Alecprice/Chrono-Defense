...
const worlds=[
 {id:'retro',towerSelector:'.retro-tower-grid button',enemySelector:'.retro-enemy>span',towers:retroTowers,enemies:retroEnemies},
 {id:'future',towerSelector:'.future-towers button',enemySelector:'.future-enemy>span',towers:futureTowers,enemies:futureEnemies},
 {id:'space',towerSelector:'.space-towers button',enemySelector:'.space-enemy>span',towers:spaceTowers,enemies:spaceEnemies},
 {id:'time-rift',towerSelector:'.rift-towers button',enemySelector:null,towers:riftTowers,enemies:[]}
];
...
function mountSprite(host,world,kind,id){if(!host||host.dataset.assetBridge===`${world}:${kind}:${id}`)return;host.dataset.assetBridge=`${world}:${kind}:${id}`;const fallback=host.textContent;const img=document.createElement('img');img.className='live-asset-sprite';img.alt='';img.draggable=false;img.src=assetPath(world,kind,id);img.addEventListener('load',()=>{host.dataset.assetLoaded='true';host.classList.add('has-live-asset')},{once:true});img.addEventListener('error',()=>{img.remove();host.classList.remove('has-live-asset');host.dataset.assetLoaded='false';if(!host.textContent.trim())host.append(document.createTextNode(fallback))},{once:true});host.prepend(img);}
function enhanceWorld(config){const byName=new Map(config.towers.map(item=>[item.name,item]));document.querySelectorAll(config.towerSelector).forEach(button=>{const item=byName.get(button.querySelector('b')?.textContent?.trim());const host=button.querySelector(':scope > span');if(item&&host)mountSprite(host,config.id,'towers',item.id)});if(config.enemySelector){const icons=uniqueIconMap(config.enemies);document.querySelectorAll(config.enemySelector).forEach(host=>{const raw=[...host.childNodes].find(node=>node.nodeType===Node.TEXT_NODE)?.textContent?.trim()||host.textContent.trim();const item=icons.get(raw);if(item)mountSprite(host,config.id,'enemies',item.id)})}}
