...
export function AppStatus(){
  ...
  useEffect(()=>{
    ...
    const onWorkerMessage=event=>{if(event.data?.type==='CHRONO_OFFLINE_READY')markReady(event.data?.cache)};
    ...
    const verifyCache=async()=>{if(disposed)return;try{if('caches'in window){const cache=await caches.open(OFFLINE_CACHE_VERSION);const ready=await cache.match(OFFLINE_READY_SENTINEL);if(ready){markReady(OFFLINE_CACHE_VERSION);return}}}catch{}if(!disposed)cacheTimer=window.setTimeout(verifyCache,500)};
