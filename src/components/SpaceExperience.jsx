...
export function SpaceExperience({onSwitchWorld}){
 ...
 return <>{screen==='campaign'?<SpaceCampaign save={save} selectedMap={selectedMap} setSelectedMap={setSelectedMap} challenge={challenge} setChallenge={setChallenge} onDaily={launchDaily} onStart={start} onBack={back} onRift={rift} onCodex={()=>setShowCodex(true)}/>:<SpaceBattleV2 key={`${selectedMap}-${challenge}-${battleKey}`} mapNumber={selectedMap} save={save} setSave={setSave} onExit={exit} onNextMap={next}/>} {!world.tutorialComplete&&<SpaceTutorial onComplete={finishTutorial}/>} {showCodex&&<AdvancedEraCodex worldId="space" title="Space Field Guide" subtitle="COLONY DEFENSE DATABASE" towers={spaceTowers} enemies={spaceEnemies} bosses={spaceBosses} onClose={()=>setShowCodex(false)}/>}</>;
}
