...
export function SpaceExperience({onSwitchWorld}){
 ...
 const start=()=>{setEraChallenge('space',challenge);setBattleKey(v=>v+1);setScreen('battle')},back=()=>onSwitchWorld?.('future'),rift=()=>onSwitchWorld?.('time-rift'),exit=()=>{setSelectedMap(Math.min(25,Math.max(1,save.worlds.space.highestMap??1)));setScreen('campaign')},next=map=>{setSelectedMap(map);setEraChallenge('space',challenge);setBattleKey(v=>v+1);setScreen('battle')},finishTutorial=()=>setSave(prev=>({...prev,worlds:{...prev.worlds,space:{...prev.worlds.space,tutorialComplete:true}}}));
 return <>{screen==='campaign'?<SpaceCampaign save={save} selectedMap={selectedMap} setSelectedMap={setSelectedMap} challenge={challenge} setChallenge={setChallenge} onStart={start} onBack={back} onRift={rift} onCodex={()=>setShowCodex(true)}/>:<SpaceBattle key={`${selectedMap}-${challenge}-${battleKey}`} mapNumber={selectedMap} save={save} setSave={setSave} onExit={exit} onNextMap={next}/>} {!world.tutorialComplete&&<SpaceTutorial onComplete={finishTutorial}/>} {showCodex&&<AdvancedEraCodex worldId="space" title="Space Field Guide" subtitle="COLONY DEFENSE DATABASE" towers={spaceTowers} enemies={spaceEnemies} bosses={spaceBosses} onClose={()=>setShowCodex(false)}/>}</>;
}
