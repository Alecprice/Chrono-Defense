...
function worldFromHash(){const value=(globalThis.location?.hash||'').replace('#','');return['stone-age','retro','future','space'].includes(value)?value:'stone-age';}
export function ChronoRouter(){
 ...
 const switchWorld=next=>{const save=loadSave();if(next==='retro'&&!save.worlds.retro?.unlocked)return;if(next==='future'&&!save.worlds.future?.unlocked)return;if(next==='space'&&!save.worlds.space?.unlocked)return;save.activeWorld=next;persistSave(save);if(globalThis.location)globalThis.location.hash=next;setWorld(next)};
