...
export function normalizeSave(parsed){
 const base=defaultSave();const stone=progress(parsed?.worlds?.['stone-age']??{},stoneAgeDefaults,'totems');const retroRaw=parsed?.worlds?.retro??{};const retro=progress(retroRaw,retroDefaults,'cartridges');retro.unlocked=Boolean(retroRaw.unlocked||stone.completedMap>=25);retro.highScore=Math.max(0,Number(retroRaw.highScore)||0);retro.bestCombo=Math.max(1,Number(retroRaw.bestCombo)||1);
 const futureRaw=parsed?.worlds?.future??{};const future=progress(futureRaw,futureDefaults,'dataCores');future.unlocked=Boolean(futureRaw.unlocked||retro.completedMap>=25);
 return{...base,...parsed,version:1,activeWorld:['stone-age','retro','future','space','time-rift'].includes(parsed?.activeWorld)?parsed.activeWorld:'stone-age',worlds:{...base.worlds,...(parsed?.worlds??{}),'stone-age':stone,retro,future},settings:{...base.settings,...(parsed?.settings??{})}};
}
