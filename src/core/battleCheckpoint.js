...
export function saveBattleCheckpoint(checkpoint){try{const value={...checkpoint,version:1,savedAt:Date.now()};storage()?.setItem(BATTLE_CHECKPOINT_KEY,JSON.stringify(value));return value}catch{return null}}
...
export function clearBattleCheckpoint(){try{storage()?.removeItem(BATTLE_CHECKPOINT_KEY)}catch{}}
