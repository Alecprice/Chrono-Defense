...
export function getSpaceLayout(mapNumber=1){const n=Math.max(1,Math.min(25,mapNumber)),region=Math.floor((n-1)/5),variant=(n-1)%5,base=patterns[(region+variant)%patterns.length],mode=(n*5+region*7+variant)%8,path=tx(base,mode),used=new Set(path),anomalyCells=tx(anomalies[(region*2+variant)%anomalies.length],mode).filter(cell=>!used.has(cell));return{path,anomalies:anomalyCells,layoutId:`space-layout-${n}`}}
