...
const routeChoices=[[0,5],[1,3],[2,1],[3,7],[4,5],[1,1],[2,7],[3,5],[4,3],[0,1],[2,5],[3,3],[4,1],[0,7],[1,5],[3,1],[4,7],[0,6],[1,7],[2,2],[4,6],[0,3],[2,3],[2,0],[3,6]];
...
export function getFutureLayout(mapNumber=1){const n=Math.max(1,Math.min(25,mapNumber)),sector=Math.floor((n-1)/5),variant=(n-1)%5,[baseIndex,mode]=routeChoices[n-1],path=tx(patterns[baseIndex],mode),used=new Set(path),nodes=tx(powerNodes[(sector*2+variant)%powerNodes.length],mode).filter(cell=>!used.has(cell));return{path,powerNodes:nodes,layoutId:`future-layout-${n}`}}
