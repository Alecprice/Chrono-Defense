...
function scale(base,mapNumber,waveNumber){
 const scale=1+(mapNumber-1)*.05+(waveNumber-1)*.075;const hp=Math.round(base.hp*scale);
 return{...base,hp,maxHp:hp,speed:base.speed*(1+Math.min(.24,(waveNumber-1)*.016)),coinReward:reward(base)};
}
export function buildRetroWave({mapNumber=1,waveNumber=1}){
 const zone=Math.min(4,Math.floor((mapNumber-1)/5));const pool=pools[zone].map(id=>byId[id]).filter(Boolean);
 const count=6+waveNumber*2+Math.floor(mapNumber/5);const units=[];
