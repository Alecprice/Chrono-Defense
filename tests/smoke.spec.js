...
test('fully precached build reloads while browser is offline',async({browser})=>{
 ...
 await page.waitForFunction(async()=>{if(!('caches'in window))return false;const cache=await caches.open('chrono-defense-shell-v30');const marker=await cache.match(new URL('/__chrono-offline-ready-v30',location.origin).href);if(!marker)return false;const data=await marker.json();for(const url of data.urls??[]){if(!(await cache.match(new URL(url,location.origin).href)))return false}return true},null,{timeout:30000});
 const missing=await page.evaluate(async()=>{const cache=await caches.open('chrono-defense-shell-v30');const marker=await cache.match(new URL('/__chrono-offline-ready-v30',location.origin).href);const data=await marker.json();const absent=[];for(const url of data.urls??[]){if(!(await cache.match(new URL(url,location.origin).href)))absent.push(url)}return absent});
